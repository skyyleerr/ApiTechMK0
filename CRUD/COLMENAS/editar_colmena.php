...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($id <= 0) {
        ...
        // Validar que la colmena pertenece al usuario
        $check = $conn->query("SELECT id_colmena FROM colmenas WHERE id_colmena=$id AND id_usuario=$id_usuario");
        if (!$check || $check->num_rows === 0) {
            ...
            $sql = "UPDATE colmenas SET nombre='$nombre', ubicacion='$ubicacion', estado='$estado' WHERE id_colmena=$id AND id_usuario=$id_usuario";
            ...
            if ($conn->query($sql)) {
                ...
                header("Location: listar_colmenas.php?success=Colmena actualizada exitosamente");
                exit;
            } else {
...
// Cargar colmena para editar
$id = isset($_GET['id']) ? intval($_GET['id']) : 0;

if ($id <= 0) {
    header("Location: listar_colmenas.php?error=ID inválido");
    exit;
}

// Verificar que la colmena pertenece al usuario
$sql = "SELECT id_colmena, nombre, ubicacion, estado, fecha_creacion FROM colmenas WHERE id_colmena=$id AND id_usuario=$id_usuario";
$resultado = $conn->query($sql);

if (!$resultado || $resultado->num_rows === 0) {
    header("Location: listar_colmenas.php?error=Colmena no encontrada o sin permiso");
    exit;
}
...
<title>Editar Colmena - ApiTech</title>
...
<h1><i class="fa-solid fa-pencil" style="color: #FFC72C;"></i> Editar Colmena</h1>
<p>Actualiza los datos de tu colmena #<?php echo $colmena['id_colmena']; ?></p>
</div>
...
Creada el <?php echo date('d/m/Y H:i', strtotime($colmena['fecha_creacion'])); ?>
</div>

<form action="editar_colmena.php" method="POST">
...
<select id="estado" name="estado" required>
    <option value="Estable" <?php echo ($colmena['estado'] === 'Estable') ? 'selected' : ''; ?>>
        🟢 Estable - Funcionando normalmente
    </option>
    <option value="Advertencia" <?php echo ($colmena['estado'] === 'Advertencia') ? 'selected' : ''; ?>>
        🟡 Advertencia - Requiere atención
    </option>
    <option value="Crítico" <?php echo ($colmena['estado'] === 'Crítico') ? 'selected' : ''; ?>>
        🔴 Crítico - Requiere acción inmediata
    </option>
</select>
<small>Selecciona el estado actual de la colmena</small>
</div>
...
<a href="listar_colmenas.php" class="btn-volver">
